...
### 仓库结构（monorepo）
```
stock_project/                    # monorepo 根目录
├── stock-fast-api/               # FastAPI 后端服务 (端口 8081)
│   ├── app/                      # Router → Schema → Service → Repository 分层
│   └── docs/                     # API 文档、数据库设计等
├── stock-front_ui/               # Vue 3 + TypeScript 前端 (端口 5173)
│   └── src/                      # Pages, Components, API layer stores
└── stock-etl-engine/             # ETL 引擎独立服务 (Docker :8001 / 内网 :8082)
    ├── app/
    │   ├── jobs/                 # 数据同步脚本（sync_*, compute_*, build_*）
    │   └── scheduler.py          # APScheduler 调度器（定时任务管理）
```

### 系统定位
| 场景 | 处理方式 |
|------|----------|
| 收盘后数据本地化 | ETL Engine 每天收盘后从 Baostock/efinance 拉取全市场当日数据，写入 PostgreSQL |
| 本地快速分析 | 选股、排名、因子计算跑在本地数据库（PostgreSQL），FastAPI 提供查询 API |
| 实时数据外部获取 | 盘中行情、盘口、分时等高时效数据继续访问外部接口 |
| 支持后续扩展 | 可扩展分钟线、龙虎榜、公告、资金流等数据域，支持按个股补历史 |

---

## 2. 系统总体设计思路
整体架构采用 **离线缓存 + 本地分析 + 实时补充** 模式：
- ETL Engine（收盘后）：把当天稳定的全市场数据同步到 PostgreSQL
- FastAPI（查询服务）：提供 REST API，供前端调用做选股、排名、统计
- Vue 3 Frontend（交互界面）：管理控制台、选股大盘、策略回测可视化
- 盘中实时：实时价格、盘口、分时等高时效数据继续访问外部数据源

### 架构优势
- ETL Engine 独立部署，调度与查询解耦，互不影响
- 查询快，不依赖外部接口做大量历史筛选
- 数据口径可控，后续策略开发更稳定
- 便于做因子预计算和快照
...
## 3. 总体架构分层
整套系统分为6个核心层级（跨三个独立进程）：

### 3.1 ETL Engine（调度 + 采集层）— `stock-etl-engine`
独立的 Docker 服务，负责定时调度和数据拉取：
- **调度**：APScheduler 管理 CronTrigger，判断交易日、控制任务执行顺序和重试
- **采集**：调用 Baostock / efinance 外部接口，处理分页、限流、重试
- **调度层职责**：判断是否交易日、控制任务执行顺序、记录任务状态
- 运行端口：Docker :8001（外网）/ :8082（内网通信）

### 3.2 标准化处理层 — ETL Engine 内部模块
负责把不同来源的数据转换成内部统一标准：
- **股票代码标准化**：`600519` → `600519.SH`
- **日期格式统一**、字段命名统一、单位换算统一（"亿元"→"元"）
- **空值和异常值处理**：`--` → null

### 3.3 数据存储层 — PostgreSQL
负责把清洗后的数据写入本地数据库：
- 幂等写入、upsert更新、表间数据组织、索引管理、数据覆盖范围维护
- 核心表：16 张（dwd_*, mart_*）

### 3.4 FastAPI 查询服务层 — `stock-fast-api`
给前端提供统一的 REST API：
- **派生计算**：基于已落库的原始事实数据计算指标（均线、RSI/MACD/ATR）
- **查询服务**：RESTful 接口供 Vue Frontend 调用，42+ 个端点
- 对外端口：8081

### 3.5 前端交互层 — `stock-front_ui`
Vue 3 + Element Plus + ECharts SPA：
- 管理控制台、选股大盘、策略回测可视化
- 通过 Axios 调用 FastAPI `/api/v1/*` 接口

### 3.6 实时数据补充层（外部）
盘中行情、盘口、分时等高时效数据继续访问外部接口（Baostock/efinance），不经过 ETL Engine。
